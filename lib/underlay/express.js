...
module.exports = function(env) {
  ...
  this.__router.define(function(method, path, handler, options) {
    var constraint, _next = true;
      ...
      app[method](function (req, res, next) {
        if ('object' === typeof constraint) {
          ...
          for (key in constraint) {
            ...
            if ('ip' === key) {
              if (-1 === val.indexOf(req.ip))
                _next = false;
            } else if ('subdomain' === key) {
              if (-1 === val.indexOf(req.subdomains.reverse.join('.')))
                _next = false;
            }
          }
        } else if ('function' === typeof constraint) {
          if (true !== constraint(req)) {
            _next = false;
          }
        }
        if (_next) next();
